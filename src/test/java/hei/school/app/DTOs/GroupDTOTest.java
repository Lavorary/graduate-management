...
class GroupDTOTest extends FacadeIT {
  ...
  void should_create_group_dto_from_entity() {
    ...
    JGroup saved = groupRepository.save(JGroup.builder().ref("G1").cursus(Set.of(cursus)).build());
  ...
  void should_build_group_dto_with_builder() {
    ...
    GroupDTO dto = GroupDTO.builder().id(id).ref("G2").cursusIds(Set.of(cursusId)).build();
