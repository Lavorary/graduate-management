...
class UserRepositoryTest extends FacadeIT {
  ...
  void should_find_graduates_by_cursus_id() {
    ...
    gradeRepository.save(JGrade.builder().exam(exam).student(student1).gradedBy(teacher).build());

    gradeRepository.save(JGrade.builder().exam(exam).student(student2).gradedBy(teacher).build());
  ...
  void should_return_empty_list_when_no_graduates_for_cursus() {
    ...
    JCursus cursus =
        cursusRepository.save(
            JCursus.builder().name("EmptyCursus").description("No students").year("2026").build());
  ...
  void should_return_empty_list_when_cursus_has_no_grades() {
    ...
    JCursus cursus =
        cursusRepository.save(
            JCursus.builder().name("NoGrades").description("No grades yet").year("2026").build());
  ...
  void should_find_graduates_without_duplicates_when_multiple_courses() {
    ...
    gradeRepository.save(JGrade.builder().exam(exam1).student(student).gradedBy(teacher).build());

    gradeRepository.save(JGrade.builder().exam(exam2).student(student).gradedBy(teacher).build());

    var graduates = userRepository.findGraduatesByCursusId(cursus.getId());

    assertThat(graduates).hasSize(1).extracting(JUser::getEmail).containsExactly("john@hei.school");
  }
}
